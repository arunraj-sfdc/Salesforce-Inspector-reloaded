...
export async function dataImportTest(test) {
  ...
  assertEquals({Queued: 3, Processing: 1, Succeeded: 0, Failed: 0}, vm.importCounts());
  assertEquals([["Name", "Checkbox__c", "Number__c", "Lookup__r:Inspector_Test__c:Name", "__Status", "__Id", "__Action", "__Errors"], ["test3", "false", "300.03", "", "Processing", "", "", ""], ["test4", "false", "400.04", "test1", "Queued", "", "", ""], ["test5", "true", "500.05", "", "Queued", "", "", ""], ["test6", "true", "600.06", "notfound", "Queued", "", "", ""]], vm.importTableResult.table);
  assertEquals(false, vm.importTableResult.isTooling);
  assertEquals([true, true, true, true, true], vm.importTableResult.rowVisibilities);
  ...
  records = getRecords(await sfConn.rest(`/services/data/v${apiVersion}/query/?q=${encodeURIComponent("select Name, Checkbox__c, Number__c, Lookup__r.Name from Inspector_Test__c order by Name")}`));
  assertEquals([
  ...
  // Update csv
  records = getRecords(await sfConn.rest(`/services/data/v${apiVersion}/query/?q=${encodeURIComponent("select Id from Inspector_Test__c order by Name")}`));
  vm.dataFormat = "csv";
  vm.didUpdate();
  vm.importAction = "update";
  vm.didUpdate();
  ...
  records = getRecords(await sfConn.rest(`/services/data/v${apiVersion}/query/?q=${encodeURIComponent("select Name from Inspector_Test__c order by Name")}`));
  assertEquals([
